...
def test_upload_conversation(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.upload_conversation),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_upload_conversation_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.ContactCenterInsightsRestInterceptor, "post_upload_conversation") as post, \
         mock.patch.object(transports.ContactCenterInsightsRestInterceptor, "pre_upload_conversation") as pre:
        pre.assert_not_called()
        post.assert_not_called()
